...
typedef enum {
    WaveConfig_Doit    = 0,
    WaveConfig_Success = 1,
    WaveConfig_Failure = 2,
} WaveConfig_e;
...
typedef enum {
    WaveAlign_12Bit_Right = 0,  // 64
    WaveAlign_12Bit_Left  = 1,  // 64
    WaveAlign_8Bit_Right  = 2,  // 128
} WaveAlign_e;

typedef __packed struct {
    u16 u16WaveConfig;     ///< 000 触发配置
    u32 u32WaveFreq;       ///< 001 波形期望频率
    u32 u32WaveRealFreq;   ///< 001 波形实际频率
    u16 u16WaveType;       ///< 002 波形类型
    u16 u16WaveAmplitude;  ///< 003 波形振幅
    u16 u16WaveAlign;      ///< 004 数据点对齐方式
    u16 u16WaveSize;       ///< 005 数据点数量
    u16 u16WaveData[64];   ///< 006 数据点数值
} WaveGen_t;
